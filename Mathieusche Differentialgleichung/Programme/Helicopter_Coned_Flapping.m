...
u1 = [ones(size(t))*deg2rad; zeros(size(t))];
% Case 2: QC = 0, QS = 1 deg
u2 = [zeros(size(t)); ones(size(t))*deg2rad];

sys = ss(A, B, eye(size(V, 2)), 0);
[y1, t1] = lsim(sys, u1, t);
[y2, t2] = lsim(sys, u2, t);
...
%% 6. Plotting inflow (States in Degrees)
figure(3);
subplot(2,1,1);
