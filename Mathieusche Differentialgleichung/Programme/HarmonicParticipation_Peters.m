...
% -----------------------------------------------------------------------
w_sq = 0.7^2;    % Natural frequency squared (w=0.3 to match figure 7)
w = sqrt(w_sq);  % Natural frequency w = 0.3
w_str = sprintf('%2.1f',w); % String to print w into the title
...
for k = 1:N_eps
    ...
    [V_unsorted, Lambda_mat_unsorted] = eig(Phi_T);
    Lambda_unsorted = diag(Lambda_mat_unsorted);
    [~,idxLambda] = sort(real(Lambda_unsorted));
    Lambda = Lambda_unsorted(idxLambda);
    V = V_unsorted(:,idxLambda);
...
hold on;
title(['Modal Participation, $\omega = $', w_str,' (Mode tracked from -',w_str,')'], 'Interpreter', 'latex');
...
jump_threshold = 1000.2; % You can adjust this if neededclose cloc
unique_m = unique(all_data_matrix(:, 3));

if useK, strK = 'k:'; else, strK = ':'; end %
...
for m_val = unique_m'
    ...
    plot(eps_nan, phi_nan, strK, 'LineWidth', 1.5);
end

if ~useK, legend(num2str(unique_m)); end
...
annotation('textbox', [0.1 0.01 0.8 0.05], 'String', ...
    ['Figure 7. Harmonic participation for w=',w_str,' (Numerical Simulation)'], ...
