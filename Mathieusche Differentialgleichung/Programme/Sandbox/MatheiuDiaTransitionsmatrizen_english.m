...
plot2D = 0; % 1: 2D and 3D plots, 0: only 3D plots created
if isInterpTex == 1 || verLessThan('matlab', '9.8') 
    isInterpTex = 1; % For R2007, the Tex interpreter is switched on            
    strInterp = 'tex';
else
    isInterpTex = 0;
    strInterp = 'latex';
...
strP.si = '$\psi \; [rad]$';
strP.ystr = 'State variables (Zustandsgrößen)';
if isInterpTex == 1
    strP.hi = '\phi [-]';
    strP.hiDot = '\phi^* [-]';
    strP.si = '\psi [rad]';
...
for dIdx = 1 %:3 %: length(DVec)
    ...
    y2 = deval(sol2,tspan);
    Pos1 = y1(1,:)' ;
    PosGesch1 = y1(2,:)' ; % Note: This is velocity (Geschwindigkeit) for the first solution
    Pos2 = y2(1,:)' ;
    Gesch2 = y2(2,:)'; % Note: This is velocity (Geschwindigkeit) for the second solution
    ...
    if plot2D == 1
        ...
        noF = plot3D(noF,tspan,PosGesch1,Pos1,T,AxisLimitsYZ,strP,labelPos,titleStr,pngfile,plot2D,isInterpTex);
    end
    pngname3D = strrep(pngname,'2D','3D');
    pngfile = fullfile(fDir,pngname3D);
    [noF,hAxis] = plot3D(noF,tspan,PosGesch1,Pos1,T,AxisLimitsYZ,strP,labelPos, titleStr,pngfile,0,isInterpTex);
    daspect([1 1 1]);
    if isInterpTex == 0
...
if plot2D % 2D: Default x,y-label, Legend
    plot(tspan,PosGesch1,tspan,Gesch1,'--')
...
if plot2D
    ...
    if isInterpTex == 0 % only use this if the interpreter is Latex
        ...
        xlabel(hAxis,strP.si,'interpreter',strInterp,'Position',labelPos.x)
        ylabel(hAxis,strP.hi,'interpreter',strInterp,'Position',labelPos.y)
        zlabel(hAxis,strP.hiDot,'interpreter',strInterp,'Position',labelPos.z,'Rotation',0)
    else
        xlabel(strP.si)
        ylabel(strP.hiDot)
        zlabel(strP.hiDot)
